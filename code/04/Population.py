class Population():
    from LinesGenotype import LinesGenotype
    ...
    def generate_population(self):
        ...
        for i in range(self.n_genotypes):
            genotypes.append(self.LinesGenotype(n_genes=self.n_genes, n_bases=self.n_bases, name="0.%i"%i))
    ...
    def generate_canvas(self,genotype):
        lines = genotype.eval(rainbow=True)
        blank_canvas = self.Canvas(self.fitness_measure.canvas.width,
                                   self.fitness_measure.canvas.height)
        for line in lines:
            blank_canvas.add_line(line)
